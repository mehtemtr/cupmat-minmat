...
async function run() {
  ...
  console.log("All opinion polls (correct_option_index = -1):");
  polls.filter(p => p.correct_option_index === -1).forEach(p => {
    console.log(`- ID: ${p.id}, Question: ${p.question}, Options: ${JSON.stringify(p.options)}`);
  });
}
