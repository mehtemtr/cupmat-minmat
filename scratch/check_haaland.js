...
async function run() {
  try {
    const { data: stats, error } = await supabase
      ...
      .eq('player_id', 'd8c8d539-0644-4ccd-8fa6-69a47ba2ea96');
    ...
    console.log(`Found ${stats.length} records for Erling Haaland:`);
    let totalGoals = 0;
    stats.forEach(s => {
      console.log(`- Stage: ${s.stage}, Goals: ${s.goals}`);
      totalGoals += s.goals || 0;
    });
    console.log("Total goals for Haaland in DB:", totalGoals);
  } catch (err) {
