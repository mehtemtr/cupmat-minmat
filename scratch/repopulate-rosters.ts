...
async function main() {
  ...
  console.log("Deleting all records from team_rosters...");
  const { error: deleteError } = await supabaseAdmin
    .from("team_rosters")
    .delete()
    .neq("team_id", "EXCLUDED_PLACEHOLDER"); // deletes all

  if (deleteError) {
    console.error("Failed to clear team_rosters:", deleteError);
    return;
  }
  console.log("Successfully cleared team_rosters.");

  console.log("Triggering updateTeamRosters to repopulate all teams (force=true)...");
  const result = await updateTeamRosters(true);
  console.log("Repopulate completed!", result);
}
