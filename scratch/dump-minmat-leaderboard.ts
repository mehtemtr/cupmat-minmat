...
async function main() {
  ...
  console.log("Fetching all minmat_leaderboard rows...");
  const { data, error } = await supabaseAdmin.from("minmat_leaderboard").select("*");
  if (error) {
    console.error("Error:", error);
    return;
  }

  const outPath = path.join(__dirname, "minmat-leaderboard-all.json");
  fs.writeFileSync(outPath, JSON.stringify(data, null, 2), "utf8");
  console.log(`Successfully dumped ${data.length} rows to ${outPath}`);
}
