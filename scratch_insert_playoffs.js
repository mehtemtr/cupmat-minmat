...
const rawData = fs.readFileSync('raw_matches_playoff.txt', 'utf8');
...
async function run() {
    try {
        let { data: existing } = await supabase.from('cupmat_tournaments').select('id, api_id').eq('api_id', 848).single();
        if (!existing) {
            console.error("Tournament UECL not found!");
            return;
        }
        ...
        for (const line of lines) {
            if (line === "Konferans Ligi") continue;
            if (line === "Play-off") {
                currentRound = line;
                continue;
            }
            ...
            if (status.includes(':')) {
                ...
                parsedMatches.push({
                    api_id: 9930000 + parsedMatches.length,
                    tournament_id: tournamentId,
                    season: 2026,
                    round: currentRound,
                    date: new Date(parseDateStr(currentDate, status)),
                    status: "NS", // Not Started
                    home_team_id: 8300 + parsedMatches.length,
                    home_team_name: home_team,
                    home_team_country_code: null,
                    home_score: null,
                    home_score_90: null,
                    away_team_id: 9300 + parsedMatches.length,
