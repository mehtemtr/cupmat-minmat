-- 1. Ensure unique constraint on profiles(user_id) exists so we can reference it
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 2. POLLS & TRIVIA QUESTIONS TABLE
CREATE TABLE IF NOT EXISTS public.polls (
    ...
    correct_option_index INTEGER NOT NULL DEFAULT -1, -- -1 means opinion poll, >=0 means trivia correct index
    points_reward INTEGER DEFAULT 10 NOT NULL,
    active_until TIMESTAMP WITH TIME ZONE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL
);

-- 3. POLL SUBMISSIONS TABLE
...
-- 4. PRIVATE LEAGUES TABLE
CREATE TABLE IF NOT EXISTS public.private_leagues (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    name TEXT NOT NULL,
    created_by TEXT REFERENCES public.profiles(user_id) ON DELETE CASCADE,
...
-- 5. PRIVATE LEAGUE MEMBERS TABLE
