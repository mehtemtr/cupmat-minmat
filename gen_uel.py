...
COUNTRY_TR_MAP = {
    'cy': 'KIB', 'am': 'ERM', 'gr': 'YUN', 'cz': 'ÇEK', 'pl': 'POL',
    'nl': 'HOL', 'be': 'BEL', 'fr': 'FRA', 'pt': 'POR', 'tr': 'TÜR',
    'at': 'AVU', 'eng': 'İNG', 'sct': 'İSK', 'de': 'ALM', 'it': 'İTA',
    'es': 'İSP', 'hr': 'HIR', 'il': 'İSR', 'no': 'NOR', 'si': 'SLO',
    'hu': 'MAC', 'bg': 'BUL'
}
...
for i, r in enumerate(rows[3:]):
    ...
    home_meta = teams_meta.get(ev, {'id': 90000 + i, 'name': ev, 'logo': None, 'country_code': COUNTRY_TR_MAP.get(ev_ulke, ev_ulke.upper())})
    away_meta = teams_meta.get(dep, {'id': 95000 + i, 'name': dep, 'logo': None, 'country_code': COUNTRY_TR_MAP.get(dep_ulke, dep_ulke.upper())})
    
    match_record = {
        'api_id': base_api_id + i,
        'tournament_id': 'd64eca96-87d8-4675-8050-93bbcf746634',
        'season': 2026,
...
with open('uel_matches_prepared.json', 'w', encoding='utf-8') as f:
    json.dump(matches_to_insert, f, ensure_ascii=False, indent=2)

print(f'Successfully generated uel_matches_prepared.json with {len(matches_to_insert)} matches.')
