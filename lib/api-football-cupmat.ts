...
export async function fetchAndStoreTournamentSeasonMatches(leagueId: number, season: number = 2024) {
  ...
  try {
    ...
    for (const item of allFixtures) {
      ...
      if (!tournamentId) {
        const region = LEAGUE_REGIONS[league.id] || 'europe';
        const { data: newTournament, error: tError } = await supabaseAdmin
          .from("cupmat_tournaments")
          .insert({
            api_id: league.id,
      ...
      const matchData = {
        ...
        home_team_id: teams.home.id,
        home_team_name: teams.home.name,
        home_team_country_code: getTeamCountryCode(teams.home.name),
        home_team_logo: teams.home.logo,
        home_score: goals.home,
        ...
        away_team_id: teams.away.id,
        away_team_name: teams.away.name,
        away_team_country_code: getTeamCountryCode(teams.away.name),
        away_team_logo: teams.away.logo,
        away_score: goals.away,
        away_penalty_score: score.penalty?.away || null,
        away_is_winner: teams.away.winner,
        
        home_score_90: score.fulltime?.home || null,
        away_score_90: score.fulltime?.away || null,
    ...
    await Promise.all(promises);
    logs.push(`[DB] Successfully loaded season matches. Inserted: ${inserted}, Updated: ${updated}`);
    return { success: true, inserted, updated, logs };
