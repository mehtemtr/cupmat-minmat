...
const TEAM_COUNTRIES: Record<string, string> = {
  ...
  // Fransa
  'Lyon': 'FRA', 'PSG': 'FRA', 'Paris Saint-Germain': 'FRA', 'Lille': 'FRA', 'Monaco': 'FRA', 'Marseille': 'FRA', 'Nice': 'FRA', 'Lens': 'FRA', 'Rennes': 'FRA', 'Brest': 'FRA',
  'Angoulême': 'FRA', 'Angouleme': 'FRA', 'Angoulême Charente FC': 'FRA', 'Bayonne': 'FRA', 'Aviron Bayonnais': 'FRA',
  'Dinan Léhon': 'FRA', 'Dinan Lehon': 'FRA', 'Chauray': 'FRA', 'FC Chauray': 'FRA', 'Lorient II': 'FRA', 'Lorient B': 'FRA',
  'Pontivy GSI': 'FRA', 'GSI Pontivy': 'FRA', 'Le Poiré sur Vie': 'FRA', 'Le Poire sur Vie': 'FRA', 'Le Poiré-sur-Vie': 'FRA',
  'Les Herbiers': 'FRA', 'Saumur': 'FRA', 'Olympique de Saumur': 'FRA', 'Saint-Colomban Locminé': 'FRA', 'Saint-Colomban Locmine': 'FRA',
  'Stade Briochin': 'FRA', 'Tarbes': 'FRA', 'Tarbes Pyrénées': 'FRA', 'Avranches': 'FRA', 'US Avranches': 'FRA', 'Saint-Malo': 'FRA', 'US Saint-Malo': 'FRA',
  'Granville': 'FRA', 'US Granville': 'FRA', 'Châteaubriant': 'FRA', 'Chateaubriant': 'FRA',
  // Yunanistan
  'AEK Athens FC': 'YUN', 'AEK Athens': 'YUN', 'Olympiakos': 'YUN', 'PAOK': 'YUN', 'Panathinaikos': 'YUN', 'Aris': 'YUN', 'OFI': 'YUN', 'OFI Crete': 'YUN',
  // Bulgaristan
  ...
  // İsviçre
  'Young Boys': 'İSV', 'Servette': 'İSV', 'Lugano': 'İSV', 'FC Lugano': 'İSV', 'St. Gallen': 'İSV', 'Thun': 'İSV', 'FC Thun': 'İSV', 'Zurich': 'İSV', 'Sion': 'İSV', 'FC Sion': 'İSV', 'Vaduz': 'İSV', 'FC Vaduz': 'İSV',
  // Belçika
  'Club Brugge': 'BEL', 'Union SG': 'BEL', 'Union Saint-Gilloise': 'BEL', 'Anderlecht': 'BEL', 'Gent': 'BEL', 'KAA Gent': 'BEL', 'Cercle Brugge': 'BEL', 'Antwerp': 'BEL', 'Genk': 'BEL', 'St. Truiden': 'BEL',
  // Portekiz
  'Sporting CP': 'POR', 'Benfica': 'POR', 'Porto': 'POR', 'Braga': 'POR', 'Vitoria Guimaraes': 'POR', 'Santa Clara': 'POR', 'Torreense': 'POR',
  // İspanya
  'Real Madrid': 'İSP', 'Barcelona': 'İSP', 'Barça': 'İSP', 'Atlético Madrid': 'İSP', 'Real Sociedad': 'İSP', 'Athletic Bilbao': 'İSP', 'Girona': 'İSP', 'Real Betis': 'İSP', 'Sevilla': 'İSP', 'Villarreal': 'İSP', 'Celta Vigo': 'İSP', 'Getafe': 'İSP',
  // İngiltere
  'Man City': 'İNG', 'Manchester City': 'İNG', 'Arsenal': 'İNG', 'Liverpool': 'İNG', 'Aston Villa': 'İNG', 'Tottenham': 'İNG', 'Chelsea': 'İNG', 'Man United': 'İNG', 'Manchester United': 'İNG', 'Newcastle': 'İNG', 'West Ham': 'İNG', 'Sunderland': 'İNG', 'Crystal Palace': 'İNG', 'Bournemouth': 'İNG', 'Brighton': 'İNG',
  // Almanya
  'Bayer Leverkusen': 'ALM', 'Bayern Munich': 'ALM', 'Stuttgart': 'ALM', 'VfB Stuttgart': 'ALM', 'RB Leipzig': 'ALM', 'Borussia Dortmund': 'ALM', 'Eintracht Frankfurt': 'ALM', 'Hoffenheim': 'ALM', 'TSG Hoffenheim': 'ALM', 'Heidenheim': 'ALM', 'SC Freiburg': 'ALM', 'Freiburg': 'ALM',
  // İtalya
  ...
  // Danimarka
  'FC Midtjylland': 'DAN', 'Midtjylland': 'DAN', 'Brondby': 'DAN', 'FC Copenhagen': 'DAN', 'Kopenhag': 'DAN', 'FC Nordsjaelland': 'DAN', 'Nordsjaelland': 'DAN', 'Silkeborg': 'DAN', 'Aarhus': 'DAN', 'AGF Aarhus': 'DAN',
  // İsveç
  'Malmö FF': 'İSVE', 'Malmo': 'İSVE', 'Elfsborg': 'İSVE', 'BK Häcken': 'İSVE', 'Hacken': 'İSVE', 'Djurgarden': 'İSVE', 'Hammarby': 'İSVE', 'Mjallby': 'İSVE', 'Mjällby AIF': 'İSVE', 'Mjallby AIF': 'İSVE', 'Göteborg': 'İSVE',
  // Sırbistan
  'Kızılyıldız': 'SIR', 'FK Crvena Zvezda': 'SIR', 'Partizan': 'SIR', 'FK Partizan': 'SIR', 'FK Vojvodina': 'SIR', 'TSC Backa Topola': 'SIR', 'Radnicki 1923': 'SIR',
  // Romanya
  ...
  'KI Klaksvik': 'FAR', 'Klaksvik': 'FAR', 'HB Torshavn': 'FAR', 'B36 Torshavn': 'FAR', 'Vikingur Gota': 'FAR', 'Runavik': 'FAR', 'NSÍ Runavík': 'FAR',
  ...
  'RFS': 'LET', 'Riga FC': 'LET', 'Riga': 'LET', 'FK Auda': 'LET', 'Liepaja': 'LET', 'FK Liepaja': 'LET', 'FK Jelgava': 'LET', 'Rigas Skola': 'LET',
  ...
  'Struga': 'K.MK', 'Shkendija': 'K.MK', 'FK Shkendija': 'K.MK', 'Tikves': 'K.MK', 'Vardar': 'K.MK',
  // Güney Amerika
  'Boca Juniors': 'ARG', 'River Plate': 'ARG', 'Estudiantes': 'ARG', 'Estudiantes L.P.': 'ARG', 'Rosario C.': 'ARG', 'Rosario Central': 'ARG', 'Rosario': 'ARG', 'ROS': 'ARG', 'Argentinos Jr.': 'ARG', 'Atl Lanus': 'ARG', 'Platense': 'ARG', 'Ind. Mendoza': 'ARG', 'Tigre': 'ARG', 'Talleres': 'ARG', 'Racing Club': 'ARG', 'San Lorenzo': 'ARG', 'Godoy Cruz': 'ARG', 'Defensa': 'ARG', 'Velez Sarsfield': 'ARG', 'Huracan': 'ARG', 'Lanus': 'ARG', 'Lanús': 'ARG', 'Rivadavia': 'ARG', 'Sportivo Rivadavia': 'ARG', 'Ind. Rivadavia': 'ARG',
  'Flamengo': 'BRA', 'Palmeiras': 'BRA', 'Corinthians': 'BRA', 'Fluminense': 'BRA', 'Botafogo': 'BRA', 'Botafogo RJ': 'BRA', 'Cruzeiro': 'BRA', 'Cruzeiro MG': 'BRA', 'Atletico-MG': 'BRA', 'Atlético Mineiro': 'BRA', 'Bahia BA': 'BRA', 'Mirassol SP': 'BRA', 'Mirassol': 'BRA', 'Santos': 'BRA', 'Vasco DA Gama': 'BRA', 'Vasco da Gama': 'BRA', 'Vitoria': 'BRA', 'EC Vitória': 'BRA', 'RB Bragantino': 'BRA', 'Internacional': 'BRA', 'Gremio': 'BRA', 'Grêmio': 'BRA', 'Sao Paulo': 'BRA', 'Athletico-PR': 'BRA', 'Fortaleza': 'BRA', 'Cuiaba': 'BRA', 'Juventude': 'BRA', 'Atletico GO': 'BRA',
  ...
  'Medellin': 'KOL', 'Independiente Medellin': 'KOL', 'Independiente': 'KOL', 'Santa Fe': 'KOL', 'Independiente Santa Fe': 'KOL', 'Dep. Tolima': 'KOL', 'Deportes Tolima': 'KOL', 'Tolima': 'KOL', 'TOL': 'KOL', 'Atl. Junior': 'KOL', 'Junior': 'KOL', 'Millonarios': 'KOL', 'Atletico Nacional': 'KOL', 'America de Cali': 'KOL', 'Deportivo Cali': 'KOL', 'Once Caldas': 'KOL', 'Aguilas Doradas': 'KOL', 'Deportivo Pereira': 'KOL', 'La Equidad': 'KOL', 'Bucaramanga': 'KOL',
  'LDU Quito': 'EKV', 'LDU de Quito': 'EKV', 'Ind. del Valle': 'EKV', 'Independiente del Valle': 'EKV', 'Barcelona SC': 'EKV', 'U. Catolica': 'EKV', 'Uni. Catolica': 'EKV', 'Univ Católica': 'EKV', 'CAT': 'EKV', 'Universidad Catolica del Ecuador': 'EKV', 'Emelec': 'EKV', 'El Nacional': 'EKV', 'Aucas': 'EKV', 'Delfin': 'EKV', 'Macara': 'EKV', 'Deportivo Cuenca': 'EKV',
  ...
  'Alianza Lima': 'PER', 'U. Deportes': 'PER', 'Universitario': 'PER', 'S. Cristal': 'PER', 'Sporting Cristal': 'PER', 'Cristal': 'PER', 'Cusco': 'PER', 'Cienciano': 'PER', 'Melgar': 'PER', 'FBC Melgar': 'PER', 'ADT Tarma': 'PER', 'Los Chankas': 'PER', 'Cesar Vallejo': 'PER', 'Sport Huancayo': 'PER',
  ...
  'Dep. Tachira': 'VEN', 'Deportivo Tachira': 'VEN', 'Carabobo': 'VEN', 'Dep. La Guaira': 'VEN', 'Deportivo La Guaira': 'VEN', 'UCV': 'VEN', 'Universidad Central': 'VEN', 'Academia Puerto Cabello': 'VEN', 'Puerto Cabello': 'VEN', 'Metropolitanos': 'VEN', 'Caracas': 'VEN', 'Monagas': 'VEN', 'Portuguesa': 'VEN',
  'UNAM': 'MEX', 'Pumas UNAM': 'MEX', 'Pumas': 'MEX',
  // Afrika (CAF)
  'AFAD': 'CIV', 'Académie de Football Amadou Diallo': 'CIV', 'ASEC Mimosas': 'CIV', 'ASEC': 'CIV', 'San-Pédro': 'CIV', 'San Pedro': 'CIV', 'FC San-Pédro': 'CIV', 'FC San Pedro': 'CIV', 'Korhogo': 'CIV', 'CO Korhogo': 'CIV', 'Stade d Abidjan': 'CIV', 'Africa Sports': 'CIV', 'Racing Club Abidjan': 'CIV',
  'African Stars': 'NAM', 'African Stars FC': 'NAM',
  'Aigle Noir': 'BDI', 'Aigle Noir Makamba': 'BDI', 'Rukinzo': 'BDI', 'Rukinzo FC': 'BDI', 'Vital O': 'BDI',
  'Aiglons': 'TCD', 'Aiglons FC': 'TCD', 'AS PSI': 'TCD', 'Gazelle FC': 'TCD', 'Elect Sport': 'TCD',
  'Al Ahli Wad Medani': 'SDN', 'Al Hilal Port Sudan': 'SDN', 'Al Hilal Omdurman': 'SDN', 'Al Hilal': 'SDN', 'Al Merreikh': 'SDN', 'Al-Merrikh': 'SDN', 'Al Merrikh': 'SDN',
  'Al Ahli Tripoli': 'LBY', 'Asswehly': 'LBY', 'Al Hilal Benghazi': 'LBY', 'Al Nasr Benghazi': 'LBY', 'Al Ittihad Tripoli': 'LBY',
  'Al-Ghazal': 'SSD', 'EL Merriekh Bentiu': 'SSD', 'El Merriekh Bentiu': 'SSD', 'Al Merreikh Juba': 'SSD', 'Jamus FC': 'SSD',
  'APR': 'RWA', 'APR FC': 'RWA', 'Rayon Sports': 'RWA', 'Rayon Sports FC': 'RWA', 'Police FC': 'RWA',
  'Black Bulls Maputo': 'MOZ', 'Black Bulls': 'MOZ', 'Songo': 'MOZ', 'UD Songo': 'MOZ', 'Costa do Sol': 'MOZ', 'Ferroviario Maputo': 'MOZ',
  'ASAS': 'DJI', 'AS Ali Sabieh': 'DJI', 'Djibouti Telecom': 'DJI', 'Port Djibouti': 'DJI', 'Arta Solar 7': 'DJI',
  'NIGELEC': 'NER', 'AS Nigelec': 'NER', 'AS ZAM': 'NER', 'AS FAN': 'NER', 'USGN': 'NER', 'Sahel SC': 'NER',
  'ASC Kara': 'TOG', 'AS Kara': 'TOG', 'ASKO Kara': 'TOG', 'Semassi': 'TOG',
  'AC Douane': 'MTN', 'AS Douanes': 'MTN', 'Nouadhibou': 'MTN', 'FC Nouadhibou': 'MTN', 'Chemal FC': 'MTN',
  'Réal Bamako': 'MLI', 'Real Bamako': 'MLI', 'Djoliba': 'MLI', 'Diarra': 'MLI', 'FC Diarra': 'MLI', 'Stade Malien': 'MLI',
  'Mangasport': 'GAB', 'AS MangaSport': 'GAB', 'CF Mounana': 'GAB', 'US Bitam': 'GAB',
  'ASPL 2000': 'MRI', 'La Cure Waves': 'MRI', 'Pamplemousses SC': 'MRI', 'Cercle de Joachim': 'MRI',
  'Petro de Luanda': 'ANG', '1º de Agosto': 'ANG', '1 de Agosto': 'ANG', 'Primeiro de Agosto': 'ANG', 'Wiliete': 'ANG', 'Wiliete SC': 'ANG', 'Kabuscorp': 'ANG', 'Sagrada Esperança': 'ANG',
  'Atomic Ngome': 'COM', 'Fomboni Club': 'COM', 'Fomboni': 'COM', 'Volcan Club': 'COM', 'US Zilimadjou': 'COM',
  'CFFA': 'MAD', 'Inate Rouge': 'MAD', 'Fosa Juniors': 'MAD', 'CNaPS Sport': 'MAD',
  'Club Africain': 'TUN', 'ES Zarzis': 'TUN', 'Esperance Tunis': 'TUN', 'Espérance de Tunis': 'TUN', 'Etoile du Sahel': 'TUN', 'CS Sfaxien': 'TUN', 'US Monastirienne': 'TUN',
  'Colombe': 'CMR', 'Panthère': 'CMR', 'Panthere': 'CMR', 'Coton Sport': 'CMR', 'Canon Yaounde': 'CMR',
  'Diambars': 'SEN', 'Star Sport Academy': 'SEN', 'Teungueth': 'SEN', 'Generation Foot': 'SEN', 'ASC Jaraaf': 'SEN',
  'Durban City': 'RSA', 'Orlando Pirates': 'RSA', 'Mamelodi Sundowns': 'RSA', 'Sundowns': 'RSA', 'Stellenbosch': 'RSA', 'Sekhukhune United': 'RSA', 'Cape Town City': 'RSA',
  'Virunga': 'COD', 'TP Mazembe': 'COD', 'Maniema Union': 'COD', 'Vita Club': 'COD', 'Saint Eloi Lupopo': 'COD',
  'East End Lions': 'SLE', 'Bo Rangers': 'SLE', 'FC Kallon': 'SLE',
  'El Kanemi Warriors': 'NGA', 'Enugu Rangers': 'NGA', 'Shooting Stars': 'NGA', 'Remo Stars': 'NGA', 'Enyimba': 'NGA', 'Rivers United': 'NGA', 'Kano Pillars': 'NGA', 'Plateau United': 'NGA',
  '15 de Agosto': 'EQG', 'Panthers': 'EQG', 'Panthers FC': 'EQG', 'Cano Sport': 'EQG',
  'Masr': 'EGY', 'ZED FC': 'EGY', 'Pyramids FC': 'EGY', 'Pyramids': 'EGY', 'Al Ahly': 'EGY', 'Zamalek': 'EGY', 'Al Masry': 'EGY', 'Modern Future': 'EGY',
  'Foresters': 'SEY', 'Bazar Brothers': 'SEY', 'Saint Louis Suns': 'SEY', 'Côte d\'Or': 'SEY',
  'Canchungo': 'GBS', 'Sporting Bissau': 'GBS', 'Benfica de Bissau': 'GBS',
  'Gaborone United': 'BOT', 'Galaxy': 'BOT', 'Jwaneng Galaxy': 'BOT', 'Township Rollers': 'BOT', 'Orapa United': 'BOT',
  'GOR Mahia': 'KEN', 'Gor Mahia': 'KEN', 'Tusker': 'KEN', 'Tusker FC': 'KEN', 'AFC Leopards': 'KEN', 'Bandari': 'KEN',
  ...
  'Maghreb Fès': 'MAR', 'Maghreb Fes': 'MAR', 'MAS Fes': 'MAR', 'Raja Casablanca': 'MAR', 'Raja CA': 'MAR', 'FAR Rabat': 'MAR', 'AS FAR': 'MAR', 'Wydad AC': 'MAR', 'RS Berkane': 'MAR',
  'Medeama': 'GHA', 'Dreams FC': 'GHA', 'Asante Kotoko': 'GHA', 'Hearts of Oak': 'GHA',
  'Medina United': 'GAM', 'Real de Banjul': 'GAM', 'Fortune FC': 'GAM',
  'Be Forward Wanderers': 'MAW', 'Mighty Wanderers': 'MAW', 'Nyasa Big Bullets': 'MAW', 'Silver Strikers': 'MAW',
  'Power Dynamos': 'ZAM', 'Red Arrows': 'ZAM', 'Red Arrows FC': 'ZAM', 'ZESCO United': 'ZAM', 'Nkana FC': 'ZAM', 'Zanaco': 'ZAM',
  'Rahimo': 'BFA', 'RC Kadiogo': 'BFA', 'Salitas FC': 'BFA', 'ASFA Yennenga': 'BFA',
  'Scotland Mabvuku': 'ZIM', 'Scotland': 'ZIM', 'Dynamos': 'ZIM', 'Highlanders': 'ZIM', 'FC Platinum': 'ZIM',
  'AS Sobemap': 'BEN', 'ASPAC': 'BEN', 'Coton FC': 'BEN', 'Buffles du Borgou': 'BEN',
  'Welwalo Adigrat Uni': 'ETH', 'Welwalo Adigrat': 'ETH', 'Sidama Bunna': 'ETH', 'Saint George': 'ETH', 'Fasil Kenema': 'ETH', 'Ethiopian Coffee': 'ETH',
  // Asya (AFC)
...
export default function CupMatMatchCenter() {
  ...
  // Avrupa Kulüp Turnuvaları (Şampiyonlar, Avrupa, Konferans Ligi) Hiyerarşik Accordion State'leri
  const [isLeagueStageOpen, setIsLeagueStageOpen] = useState<boolean>(true);
  const [selectedLeagueWeek, setSelectedLeagueWeek] = useState<string | null>("1. Hafta");
  ...
  useEffect(() => {
    if (availableWeeks.length > 0 && selectedLeagueWeek && !availableWeeks.includes(selectedLeagueWeek)) {
      setSelectedLeagueWeek(availableWeeks[0]);
    }
        ...
        {mainView === "matches" && (
          <div className="space-y-6">
            ...
            {isLoading ? (
              ...
              /* ========================================================================= */
              /* 🌟 AVRUPA KULÜP TURNUVALARI (Şampiyonlar, Avrupa & Konferans Ligi)         */
              /* Görsel 1 & 2 Tasarımı: Lig Aşaması -> 1..8 Hafta -> Maçlar & Ön Elemeler    */
              /* ========================================================================= */
              <div className="space-y-4">
                
                {/* 1. ANA TUR: LİG AŞAMASI (ACCORDION) */}
                <div className="bg-slate-900/40 border border-slate-700/50 rounded-2xl overflow-hidden shadow-sm">
                  {/* Lig Aşaması Başlığı */}
                  <button
                    onClick={() => setIsLeagueStageOpen(!isLeagueStageOpen)}
                    className="w-full flex items-center justify-between p-4 sm:px-6 bg-slate-800/30 hover:bg-slate-800/60 transition-colors cursor-pointer border-b border-slate-800/40"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-lg font-bold text-white">{t("Lig Aşaması")}</h3>
                    </div>
                    {isLeagueStageOpen ? <ChevronDown className="w-5 h-5 text-slate-400" /> : <ChevronRight className="w-5 h-5 text-slate-400" />}
                  </button>

                  {/* Lig Aşaması İçeriği (Haftalar Listesi) */}
                  {isLeagueStageOpen && (
                    <div className="p-3 sm:p-5 space-y-3 bg-slate-950/30">
                      {availableWeeks.map(week => {
                        const isWeekSelected = selectedLeagueWeek === week;
                        ...
                        return (
                          <div key={week} className="bg-slate-900/60 border border-slate-800/80 rounded-xl overflow-hidden transition-all">
                            {/* Hafta Başlık Butonu (1. Hafta, 2. Hafta...) */}
                            <button
                              onClick={() => setSelectedLeagueWeek(isWeekSelected ? null : week)}
                              className={`w-full flex items-center justify-between p-3.5 sm:px-5 transition-colors cursor-pointer ${
                                isWeekSelected ? "bg-slate-800/60" : "bg-slate-800/20 hover:bg-slate-800/40"
                              }`}
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <h4 className="text-base font-bold text-slate-100">{week}</h4>
                                <span className="text-xs text-slate-400">({weekMatches.length} {t("maç")})</span>
                              </div>
                              {isWeekSelected ? <ChevronDown className="w-4 h-4 text-indigo-400" /> : <ChevronRight className="w-4 h-4 text-slate-400" />}
                            </button>

                            {/* Seçili Haftanın Maçları */}
                            {isWeekSelected && (
                              <div className="p-3 sm:p-5 space-y-6 border-t border-slate-800/60 bg-slate-900/20">
                                {weekMatches.length === 0 ? (
                                  <p className="text-xs text-slate-400 py-4 text-center">{t("Bu hafta için henüz maç planı bulunmuyor.")}</p>
                                ) : (
                                  sortedDates.map(dateStr => (
                                    <div key={dateStr} className="space-y-3">
                                      <div className="flex items-center gap-2 border-b border-slate-800/80 pb-2">
                                        <h5 className="text-xs sm:text-sm font-semibold text-slate-400">{dateStr}</h5>
                                      </div>
                ...
                {/* 2. ELEME & PLAY-OFF VE DİĞER TURLAR (ACCORDION OLARAK LİG AŞAMASININ ALTINDA SIRALANIR) */}
                {availableKnockoutRounds.map(rName => {
                  const isRoundOpen = expandedRounds[rName] !== false;
                  ...
                  return (
                    <div key={rName} className="bg-slate-900/40 border border-slate-700/50 rounded-2xl overflow-hidden shadow-sm">
                      <button
                        onClick={() => toggleRound(rName)}
                        className="w-full flex items-center justify-between p-4 sm:px-6 bg-slate-800/30 hover:bg-slate-800/60 transition-colors cursor-pointer"
                      >
                        <div className="flex items-center gap-3">
                          <h3 className="text-lg font-bold text-white">{getLocalizedRoundName(rName, currentLocale)}</h3>
                          <span className="text-xs text-slate-400">({rMatches.length} {t("maç")})</span>
                        </div>
                        {isRoundOpen ? <ChevronDown className="w-5 h-5 text-slate-400" /> : <ChevronRight className="w-5 h-5 text-slate-400" />}
                      </button>

                      {isRoundOpen && (
                        <div className="p-4 sm:p-6 space-y-6 border-t border-slate-800/40">
                          {sortedDates.map(dateStr => {
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <div className="flex flex-col items-center justify-center min-w-[70px] sm:min-w-[90px]">
                                          {["FT", "AET", "PEN"].includes(match.status) ? (
                                            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-900/90 rounded-md border border-slate-700">
                                              <span className={`text-sm sm:text-base tabular-nums ${isTeam1Advancing ? 'font-black text-emerald-400' : 'font-bold text-slate-100'}`}>{match.team1.score}</span>
                                              <span className="text-slate-500">-</span>
                                              <span className={`text-sm sm:text-base tabular-nums ${isTeam2Advancing ? 'font-black text-emerald-400' : 'font-bold text-slate-100'}`}>{match.team2.score}</span>
                                            </div>
                                          ) : (
                                            <div className="text-xs sm:text-sm font-bold text-amber-400 bg-amber-500/10 px-2.5 py-0.5 rounded-md border border-amber-500/20">{match.time}</div>
                                          )}
